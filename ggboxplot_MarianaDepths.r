...
ggboxplot_MDepths<- ggboxplot(
	MDepths_df, 
	title = "Mariana Trench: \nNotched Boxplot for Data Groups by 25 Profiles with Outliers",
	x = "profiles", 
	y = "depths", 
#	add = "jitter",
	width = 0.8, 
...
p<- ggboxplot_MDepths + 
    ...
    axis.title.y = element_text(margin = margin(t = 20, r = .3), family = "Times New Roman", face = 2, size = 6),
    axis.title.x = element_text(family = "Times New Roman", face = 2, size = 6,margin = margin(t = .2)))

# шаг-6. сохраняем боксплот в pdf.	
ggsave("ggboxplot_MarianaDepths.pdf", device = cairo_pdf, fallback_resolution = 300)